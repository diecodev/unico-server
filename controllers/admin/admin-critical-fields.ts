...
export const adminUpdateCriticalFields = async ({ request, response, cookies }: Context) => {
  // Taking the cookie
  ...
  try {
    ...
    const { username, password, email } = await request.body({ type: "json" }).value as Partial<AdminSchema>;
