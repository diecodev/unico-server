...
export const servicesByDateDescendig = async ({ cookies, response }: Context) => {
  ...
  try {
    ...
    const services = await service_model.find({ date_of_service: { $gte: last_date, $lt: first_date } }).sort({ date_of_service: -1 }).toArray();
